...
def plot_metrics(metrics, model_type):
    ...
    plt.tight_layout()
    plt.savefig(f'{model_type}_training_validation_metrics.png', bbox_inches='tight', dpi=300)
    plt.close()

def evaluate_model(model, test_features, test_labels, model_type):
    ...
    plt.savefig(f'{model_type}_confusion_matrix.png', bbox_inches='tight', dpi=300)
    plt.close()
    ...
    plt.savefig(f'{model_type}_roc_curve.png', bbox_inches='tight', dpi=300)
    plt.close()

    # F1 Scores
    plot_f1_scores(y_true_classes, y_pred_classes, model_type, range(test_labels.shape[1]))

    # Precision-Recall Curve
    plot_precision_recall_curve(test_labels, y_pred_probabilities, model_type)
...
def save_metrics_to_csv(metrics, model_type):
    ...
    df.to_csv(f'{model_type}_training_metrics.csv', index=False)
    
    # Save total training time
    with open(f'{model_type}_total_training_time.txt', 'w') as f:
        f.write(f"Total training time: {metrics['total_training_time']:.2f} seconds")

def plot_f1_scores(y_true, y_pred, model_type, labels):
    ...
    plt.tight_layout()
    plt.savefig(f'{model_type}_f1_scores.png', bbox_inches='tight', dpi=300)
    plt.close()

def plot_precision_recall_curve(y_true, y_pred_prob, model_type):
    ...
    plt.savefig(f'{model_type}_precision_recall_curve.png', bbox_inches='tight', dpi=300)
    plt.close()

def save_model_summary(model, model_type):
    with open(f'{model_type}_model_summary.txt', 'w') as f:
        model.summary(print_fn=lambda x: f.write(x + '\n'))

def plot_training_time(metrics, model_type):
    plt.figure(figsize=(10, 6))
    ...
    plt.savefig(f'{model_type}_training_time.png', bbox_inches='tight', dpi=300)
    plt.close()
...
def compare_models(model_metrics):
    ...
    plt.savefig('model_comparison_accuracy.png', bbox_inches='tight', dpi=300)
    plt.close()
    ...
    plt.savefig('model_comparison_time.png', bbox_inches='tight', dpi=300)
    plt.close()
...
def plot_all_metrics(model_metrics):
    ...
    plt.legend()
    plt.savefig('all_models_loss_comparison.png', bbox_inches='tight', dpi=300)
    plt.close()
    ...
    plt.legend()
    plt.savefig('all_models_accuracy_comparison.png', bbox_inches='tight', dpi=300)
    plt.close()